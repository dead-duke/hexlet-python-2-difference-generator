...
def get_stylish_format(data, ident=''):
    ...
    for item in data:
        ...
        item_value = get_stylish_value(data[item].get('value'), ident)
        item_previus_value = get_stylish_value(
            data[item].get('previus_value'), ident
        )
        item_children = data[item].get('children')
        new_ident = ident + "    "
        if item_type == 'nested':
            children_value = get_stylish_format(item_children, new_ident)
            stylish_output += f'{new_ident}{item}: {children_value}\n'
        elif item_type == 'changed':
            stylish_output += f'{ident}  - {item}: {item_previus_value}\n'
            stylish_output += f'{ident}  + {item}: {item_value}\n'
        else:
            sign = types[item_type]
            stylish_output += f'{ident}  {sign} {item}: {item_value}\n'
    return stylish_output + ident + '}'


def get_stylish_value(value, ident):
